...
public:
    ...
    std::string GeniusStatusJson();
#ifdef CONFIG_GENIUS_DEVICE_CORE
    int GeniusPlaybackPriority() const { return genius_priority_.load(); }
#endif
...
private:
    ...
    DeviceState notification_return_state_ = kDeviceStateIdle;
#ifdef CONFIG_GENIUS_DEVICE_CORE
    std::string genius_pending_url_, genius_pending_title_;
    int64_t genius_pending_since_ = 0;
    std::string genius_pending_kind_, genius_active_kind_, genius_current_url_,
        genius_current_title_;
    std::string genius_resume_url_, genius_resume_title_, genius_alert_text_, genius_speech_url_,
        genius_tts_id_;
    int64_t genius_alert_deadline_ = 0;
    std::atomic<int> genius_priority_{0};
    std::mutex genius_status_mutex_;
    std::string genius_status_json_ = "{\"state\":\"idle\"}";
#endif
    void TryStartGeniusPlayback();
    void CancelGeniusLifecycle(const char* reason);
    void PublishGeniusLifecycle(const char* state);
    void GeniusNotificationFinished(bool success);
    void FinishGeniusAlert(bool success);
