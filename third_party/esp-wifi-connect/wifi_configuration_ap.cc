...
void WifiConfigurationAp::OnProvisioningValue(
    std::function<void(const std::string&, const std::string&)> callback) {
    on_provisioning_value_ = std::move(callback);
}
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // Forward optional application-specific provisioning values.
            cJSON *genius_domain_item =
                cJSON_GetObjectItemCaseSensitive(json, "genius_domain");

            if (cJSON_IsString(genius_domain_item) &&
                genius_domain_item->valuestring != nullptr &&
                this_->on_provisioning_value_) {
                this_->on_provisioning_value_(
                    "genius_domain",
                    genius_domain_item->valuestring);
            }
            cJSON_Delete(json);
            // 设置成功响应
            httpd_resp_set_type(req, "application/json");
